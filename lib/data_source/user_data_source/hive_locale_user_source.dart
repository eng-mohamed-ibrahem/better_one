...
class HiveLocaleUser extends HiveInit implements LocaleUserSource {
  @override
  Future<ResultHandler<TaskModel, Failure>> addTask(TaskModel newTask) async {
    try {
      await appBox.put(
        CacheKeys.tasks,
        _convertToJson(
          _convertToTaskList(appBox.get(CacheKeys.tasks))..add(newTask),
        ),
      );
  ...
  Future<ResultHandler<TaskModel, Failure>> getTaskById(String id) async {
    try {
      var tasks = _convertToTaskList(appBox.get(CacheKeys.tasks));
  ...
  Future<ResultHandler<TaskModel, Failure>> removeTask(
    ...
    try {
      ...
      await appBox.put(CacheKeys.tasks, _convertToJson(tasks));
